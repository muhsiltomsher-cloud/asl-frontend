...
export default function StoreLocatorClient({ dict, locale }: StoreLocatorClientProps) {
  const isRTL = locale === "ar";

  const breadcrumbItems = [
    { name: dict.heroSubtitle, href: `/${locale}/store-locator` },
  ];

  const content = {
    heroTitle: dict.heroTitle,
    heroSubtitle: dict.heroSubtitle,
    heroDescription: dict.heroDescription,
    storesCount: `${stores.length} ${dict.stores}`,
    countriesCount: `${countries.length} ${dict.countries}`,
    getDirections: dict.getDirections,
    openingHours: dict.openingHours,
    openingHoursValue: dict.openingHoursValue,
    ctaTitle: dict.ctaTitle,
    ctaSubtitle: dict.ctaSubtitle,
    ...
    <div className="flex flex-col">
        ...
        <div className="container relative mx-auto flex min-h-[50vh] items-center px-4 py-16">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mb-6 flex items-center justify-center gap-4">
              ...
              <MapPin className="h-6 w-6 text-[#c67a46]" />
            ...
            <div className="mt-10 flex items-center justify-center gap-8">
              <div className="flex items-center gap-3 rounded-full border border-[#c67a46]/30 bg-white/10 px-6 py-3 backdrop-blur-sm">
                <Building2 className="h-5 w-5 text-[#c67a46]" />
                <span className="text-sm font-medium text-[#f7f6f2]">{content.storesCount}</span>
              </div>
              <div className="flex items-center gap-3 rounded-full border border-[#c67a46]/30 bg-white/10 px-6 py-3 backdrop-blur-sm">
                <MapPin className="h-5 w-5 text-[#c67a46]" />
                <span className="text-sm font-medium text-[#f7f6f2]">{content.countriesCount}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center justify-center gap-4 md:flex-row md:gap-8">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7f6f2]">
                <Clock className="h-5 w-5 text-[#b2a896]" />
              </div>
              <div>
                <p className="text-sm font-medium text-[#633d1f]">{content.openingHours}</p>
                <p className="text-sm text-[#b2a896]">{content.openingHoursValue}</p>
              </div>
            </div>
            <div className="hidden h-8 w-px bg-[#b2a896] md:block" />
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7f6f2]">
                <Phone className="h-5 w-5 text-[#b2a896]" />
              </div>
              <div>
                <p className="text-sm font-medium text-[#633d1f]">{dict.contactUs}</p>
                <p className="text-sm text-[#b2a896]">+971 50 607 1405</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container relative mx-auto px-4">
          {countries.map((country, countryIndex) => (
            <div key={country.id} className={countryIndex > 0 ? "mt-20" : ""}>
              {/* Country Header */}
              <div className="mb-10 border-b-2 border-[#b2a896] pb-6">
                <div className="flex items-center gap-4">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#c67a46] to-[#633d1f] shadow-lg shadow-[#c67a46]/30">
                    <MapPin className="h-7 w-7 text-white" />
                  </div>
                  <div>
                    <h2 className="text-3xl font-bold uppercase text-[#633d1f] md:text-4xl">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {country.regions.flatMap((region) =>
                  region.stores.map((store) => (
                    <div
                      key={store.id}
                      className="group relative overflow-hidden bg-white shadow-lg transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#633d1f]/10"
                    >
                      {/* Store Image */}
                      <div className="relative h-48 overflow-hidden">
                        ...
                        {/* City Badge */}
                        <div className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-[#633d1f] backdrop-blur-sm">
                      ...
                      <div className="p-5">
                        <div className="mb-4 space-y-3">
                          <div className="flex items-center gap-3 text-[#b2a896]">
                        ...
                        <a
                          ...
                          className="flex w-full items-center justify-center gap-2 bg-gradient-to-r from-[#c67a46] to-[#c67a46] px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-[#c67a46]/20 transition-all duration-300 hover:from-[#c67a46] hover:to-[#633d1f] hover:shadow-xl hover:shadow-[#c67a46]/30"
                        >
                      ...
                      {/* Decorative Corner */}
                      <div className="absolute -right-8 -top-8 h-16 w-16 rounded-full bg-[#f7f6f2]/50 transition-transform duration-500 group-hover:scale-150" />
                    </div>
                  ))
                )}
              </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-[#633d1f] via-[#633d1f] to-stone-900 py-16 md:py-20">
        ...
        <div className="container relative mx-auto px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mb-8 text-lg text-[#f7f6f2]">
              {dict.waitingSubtitle}
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {countries.map((country) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center gap-2 rounded-full border border-[#c67a46]/30 bg-white/10 px-5 py-2.5 text-sm font-medium text-[#f7f6f2] backdrop-blur-sm transition-all duration-300 hover:border-[#c67a46] hover:bg-white/20"
                >
                  <MapPin className="h-4 w-4 text-[#c67a46]" />
                  {isRTL ? country.nameAr : country.name}
                  <ChevronRight className={`h-4 w-4 text-[#c67a46] transition-transform duration-300 group-hover:translate-x-1 ${isRTL ? "rotate-180 group-hover:-translate-x-1" : ""}`} />
                </a>
              ))}
            </div>
          </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#f7f6f2] to-white py-16 md:py-24">
        ...
        <div className="container relative mx-auto px-4 text-center">
          ...
          <p className="mx-auto mb-10 max-w-2xl text-lg text-[#b2a896]">
            {content.ctaSubtitle}
          </p>
          <Link
            href={`/${locale}/shop`}
            className="group inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-[#c67a46] to-[#c67a46] px-10 py-5 text-lg font-semibold text-white shadow-lg shadow-[#c67a46]/30 transition-all duration-300 hover:from-[#c67a46] hover:to-[#633d1f] hover:shadow-xl hover:shadow-[#c67a46]/40"
          >
